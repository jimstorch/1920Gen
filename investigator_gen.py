...
class Overlay(object):
    ...
    def __init__(self, width=612, height=792):
        ...
        self.c = canvas.Canvas(self.in_memory)
    ...
    def get_pdf(self):
        ...
        return PdfFileReader(self.in_memory)
...
for x in range(10):  
    #
    #   Read in the blank form
    #
    blank = PdfFileReader(open('data/1920blank.pdf','rb'),strict=False)
    page = blank.getPage(0)
    page_count = blank.getNumPages()    

    #
    #   Generate a character and overlay
    #
    c=Character1920()
    c.sex('Female')
    c.name(female_name())
    ...
    #   Merge it
    #
    page.mergePage(o.get_pdf().getPage(0))
    target.addPage(page)

    #
    #   Uncomment to include additional pages from blank
    #
    #for x in range(1, page_count):
    #    target.addPage(blank.getPage(x))
